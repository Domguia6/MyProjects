...
<header>
    ...
    try{
     ...
     /*  $sql1 = "SELECT * FROM articles WHERE CLASSE='1ères'";

       $result1 = $connexion->query($sql1);*/

      /* if (isset($_GET['id']) && !empty($_GET['id'])) {
        // Récupérer et nettoyer l'identifiant de l'article
        $article_id = filter_var($_GET['id'], FILTER_SANITIZE_NUMBER_INT);
      echo $article_id;*/
        // Requête SQL pour récupérer les informations de l'article avec son auteur
    ...
    // Exécution de la requête SQL et traitement des résultats
/*} else {
    // Gérer le cas où l'identifiant de l'article n'est pas spécifié
    echo "Identifiant de l'article non valide";
}*/
...
/*
 if ($result->num_rows > 0) {
     ...
     while ($row1 = $result->fetch_assoc()) {
        ...
        // echo "<br>Email:<a href='mailto:".$row1["EMAIL"]."'>".$row1["EMAIL"]."</a>";
        echo "<br><center><button><a href='#'>Acheter</a></button></center>";
         echo "</div >";
         $counter++;
